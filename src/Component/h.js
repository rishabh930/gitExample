  ...
  selectSquares: function () {
    ...
    squares.forEach((square, index) => {
      if (
        ...
        !square.classList.contains('init')
      ) {
...
var actions = {
  ...
  selectPiece: function (e) {
    let piece = e.target.classList.contains('fa')
      ? e.target.parentNode
      : e.target;
    // Originally Selected Piece Already Chosen Before Choosing this Piece
    let origin = !piece.parentNode.classList.contains('piece-selected')
      ? document.querySelector('.piece-selected')
      : false;
    if (!origin) {
    ...
    actions.consoleLog('[ACTION] Select Piece', {
      piece: piece,
      origin: origin,
    });
  },
  ...
  selectSquare: function (e) {
    ...
    let selectedPiece = document
      .querySelector('.piece-selected')
      .querySelector('.chess-piece');
    if (spIsSelected) {
      if (
        selectedPiece.getAttribute('data-position') !==
        square.getAttribute('data-cell')
      ) {
        actions.movePiece(e);
        actions.clearSquareSelectors();
      }
    ...
    actions.consoleLog('[ACTION] Select Square', {
      e: e,
      square: square,
      piece: selectedPiece,
    });
  ...
  movePiece: function (e) {
    e.preventDefault();
    let newSquare =
      document.querySelector('.square-selected') !== null
        ? document.querySelector('.square-selected')
        : document.querySelector('.capturable-selected') !== null
        ? document.querySelector('.capturable-selected')
        : false;
    let oldSquare = document.querySelector('.piece-selected');
    let piece = oldSquare.querySelector('.chess-piece');
    let opponentPiece =
      newSquare.querySelector('.chess-piece') !== null
        ? newSquare.querySelector('.chess-piece')
        : false;
    if (opponentPiece) {
    ...
    actions.consoleLog('[ACTION] Move Piece', {
      e: e,
      newSquare: newSquare,
      oldSquare: oldSquare,
      piece: piece,
  ...
  takePiece: function (opponentPiece) {
    ...
    if (takerColor) {
      document
        .getElementById(takerColor + '-panel')
        .querySelector('.taken-pieces')
        .appendChild(opponentPiece);
    }
  },
};
